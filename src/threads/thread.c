...
next_thread_to_run (void) 
{
  if (list_empty (&ready_list))
    ...
    /* Get thread with maximum priority to be run */
    struct thread *next = list_entry (list_max(&ready_list , comp_priority , NULL) , struct thread , elem);
...
/* Comparator used in getting maximum thread according to their priority */
bool 
